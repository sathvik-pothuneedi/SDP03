...
public class ClientController
{
	...
	@GetMapping("/addtest")
	public ModelAndView quizdemo(HttpServletRequest request)
	{
		
		ModelAndView mv = new ModelAndView("addtest", "quiz",new QuestionBank());
	...
	public String addquestionbankdemo(@ModelAttribute("quiz") QuestionBank questionbank)
	{
		trainerService.addQuestionbank(questionbank);
		return "redirect:addtest";
	}
	@GetMapping("/viewtests")
	public ModelAndView viewquizdemo(HttpServletRequest request)
	{
        HttpSession session = request.getSession();
        ModelAndView mv = new ModelAndView();
		String tuname = (String) session.getAttribute("tuname");
        ...
        List<QuestionBank> qblist = trainerService.viewallQuestionBank();
        mv.addObject("qlist", qblist);
        
        mv.setViewName("viewtests");
        
		return mv;
	}
	@GetMapping("/taketest")
	public ModelAndView studenttaketestdemo()
	{
		ModelAndView mv = new ModelAndView("taketest");
		
		List<QuestionBank> qblist = trainerService.viewallQuestionBank();
        mv.addObject("q", qblist);
    ...
    {
      HttpSession session=request.getSession();
      String suname = (String) session.getAttribute("username");
      
      List<QuestionBank> questionbanklist = trainerService.viewallQuestionBank();
      int count = 0;
      for(QuestionBank q : questionbanklist)
      {
    	  
    	  String qid = Integer.toString(q.getQuestionid());
    	  System.out.println(request.getParameter(qid));
    	  
    	  if(request.getParameter(qid).equals(q.getCorrectoption()))
    	  {
    		  count++;
    	  }
      }
      ...
      ModelAndView mv = new ModelAndView();
      mv.setViewName("viewtestresult");
      mv.addObject("totalquestions",questionbanklist.size());
      mv.addObject("score",count);
      TestScore ts = new TestScore();
      ts.setScore(count);
      ts.setSusername(suname);
      studentService.savescore(ts);
